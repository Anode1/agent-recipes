...
resolution; jars are vendored in the repo. The build is itself a dependency and a
file the agent reads and edits, so keep it small, explicit, and offline: a stock
toolchain builds the project unchanged, with no generated layer between the
command and the compiler (recipe 5).
